...
import (
	...
	"github.com/nexusflow/nexusflow/services/attachment-service/internal/handler"
...
const serviceName = "attachment-service"
...
func main() {
	...
	log.Sugar().Infow("Starting attachment-service")
	...
	minioClient, err := storage.NewMinIOClient(
		...
		"nexusflow",
		"nexusflow123",
		"nexusflow-attachments",
		false,
		log,
	)
	if err != nil {
		log.Sugar().Fatalw("Failed to connect to MinIO", "error", err)
	}
	...
	// Initialize layers
	repo := repository.NewAttachmentRepository(db, log)
	svc := service.NewAttachmentService(repo, minioClient, producer, log)
	h := handler.NewAttachmentHandler(svc, log)
	...
	// Register services
	pb.RegisterAttachmentServiceServer(grpcServer, h)
...
func runMigrations(db *sql.DB, log *logger.Logger) error {
	driver, err := postgres.WithInstance(db, &postgres.Config{
		MigrationsTable: "schema_migrations_attachment",
	})
	if err != nil {
