...
func main() {
	...
	log.Sugar().Infow("Starting project-service")
	...
	// Register services
	pb.RegisterProjectServiceServer(grpcServer, h)
	...
	// Use a different port than user-service (50051) and org-service (50052)
	// Default to 50053 if not specified
...
func runMigrations(db *sql.DB, log *logger.Logger) error {
	driver, err := postgres.WithInstance(db, &postgres.Config{
		MigrationsTable: "schema_migrations_project",
	})
	if err != nil {
